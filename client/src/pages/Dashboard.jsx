...
import {
  ...
  CircleDollarSign,
  Pencil, // NEW
  Trash, // NEW (Replaced X)
  X, // NEW (For canceling edit)
} from "lucide-react";
import moment from "moment";

function Dashboard() {
  const [active, setActive] = useState(1);
  const [editItem, setEditItem] = useState(null); // --- NEW: Edit State ---
  const { getIncomes, error, setError } = useGlobalContext();
  const navigate = useNavigate();
  const user = JSON.parse(sessionStorage.getItem("user"));
  ...
  // --- PASSING EDIT PROPS TO ALL VIEWS ---
  const displayData = () => {
    switch (active) {
      case 1:
        return (
            ...
            {menuItems.map((item) => (
              <li
                ...
                onClick={() => {
                  setActive(item.id);
                  setError(null);
                  setEditItem(null); // Clear edit when switching tabs
                }}
                className={`cursor-pointer font-medium text-lg p-3 rounded-xl transition-all flex items-center gap-3
                                    ${
                                      active === item.id
          ...
          <div className="bg-white p-4 rounded-2xl shadow-lg border border-gray-100 h-fit">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex bg-gray-100 rounded-lg p-1 border border-gray-200">
                <button
                  onClick={() => setFormType("expense")}
                  disabled={editItem !== null} // Disable toggle while editing
                  className={`px-4 py-1.5 text-sm font-bold rounded-md transition-all ${
                    formType === "expense"
                      ? "bg-red-500 text-white shadow-md"
                      : "text-gray-500 hover:bg-gray-200"
